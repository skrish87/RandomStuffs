$(document).ready(function(){
	...
	//True for 1st Player and False for 2nd Player.
	...
	$("#board").click(function(e) {
		...
		var trELement;		
		var id = tdElement[0].id;
		var rowcol = id.split("_");
		row = +(rowcol[0]);
		col = +(rowcol[1]);
		
		var selectRowToPlace = 5;
		while(boardAr[selectRowToPlace][col] != 0) {
			selectRowToPlace--;
			if (selectRowToPlace < 0)
				return;
		}
		
		var diffRow = row-selectRowToPlace;
		
		if (diffRow < 0)
			trElement = $(tdElement).parent().nextAll()[Math.abs(diffRow)-1];
		else if (diffRow > 0)
			trElement = $(tdElement).parent().prevAll()[diffRow-1];
		else 
			trElement = $(tdElement).parent();
		
		tdElement = $(trElement).children('td')[col];
		
		placeTokenInBoard(boardAr,selectRowToPlace,col,playerToggle,tdElement);
		
		if (isVictory(boardAr,selectRowToPlace,col)) {
			showVictory(playerToggle);
			return;
		}
...
function showVictory(playerToggle) {
	$("#notifyWinArea").show();
	var winText;
	if (playerToggle)
		winText = "Player 1 wins!!! Click new game to play again...";
	else
		winText = "Player 2 wins!!! Click new game to play again...";
	$("#notifyWinArea").html(winText);
	blink("#notifyWinArea");
	$("#board").unbind('click');
}
